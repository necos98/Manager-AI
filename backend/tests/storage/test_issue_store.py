...
class TestMemoryStoreIssues:
    def test_load_after_write_is_in_memory(self, proj):
        record = issue_store.IssueRecord(
            id="ms1", project_id="p1", name="MemStore 1", status="New",
            priority=3, description="desc 1", specification=None, plan=None, recap=None,
            created_at="2026-04-01T10:00:00", updated_at="2026-04-01T10:00:00",
        )
        issue_store.create_issue(proj, record)
        loaded = issue_store.load_issue(proj, "ms1")
        assert loaded is not None
        assert loaded.name == "MemStore 1"
        assert loaded.description == "desc 1"

    def test_list_issues_full_returns_all_records(self, proj):
        _seed_issue(proj, "lf1", name="FullTest", description="full desc", specification="full spec")
        issue_store.rebuild_issues_index(proj)
        _seed_issue(proj, "lf2", name="FullTest2", description="full desc 2")
        issue_store.rebuild_issues_index(proj)

        results = issue_store.list_issues_full(proj)
        assert len(results) >= 2

        matches = [r for r in results if r.id == "lf1"]
        assert len(matches) == 1
        assert matches[0].name == "FullTest"
        assert matches[0].description == "full desc"
        assert matches[0].specification == "full spec"

    def test_empty_project_returns_empty(self, proj):
        results = issue_store.list_issues(proj)
        assert results == []
