...
2. **Memory read (MUST)**. Call `memory_search(project_id, query=<3–5 keywords from issue description>)`. If hits are returned, fetch the relevant ones via `memory_get` and factor them into spec/plan/implementation. Skip only for trivial issues (typo fix, rename).
